 ...
 * $Log: aula0903.c,v $
 * Revision 1.1  2021/10/12 04:20:42  rafael.jesus
 * Initial revision
 *
 */
...
#define ERRO_CONVERTER_ARQUIVO_FORMATO_UNIX_PARA_FORMATO_DOS				3
...
main (int argc, char *argv [ ])
{
	...
	if (argc == NUMERO_ARGUMENTOS){
		...
		nomeArquivoGerado = (char *) malloc (COMPRIMENTO_MAX_NOME_ARQUIVO * sizeof (char));
	...
	codigoRetorno = ConverterArquivoFormatoUnixParaFormatoDos (nomeArquivo, nomeArquivoGerado);
	if (codigoRetorno != ok)
	{
		printf ("Funcao ConverterArquivoFormatoUnixParaFormatoDos retornou o erro: %i \n", codigoRetorno);
		exit (ERRO_CONVERTER_ARQUIVO_FORMATO_UNIX_PARA_FORMATO_DOS);
	}
