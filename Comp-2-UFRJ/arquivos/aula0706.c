 ...
 * $Log: aula0706.c,v $
 * Revision 1.1  2021/09/21 03:31:22  rafael.jesus
 * Initial revision
 *
 */
...
#define ERRO_FUNCAO_PREENCHER_POLIGONO					5
#define ERRO_FUNCAO_LIMPAR_MONITOR							6
...
main (int argc, char *argv [])
{
	...
	unsigned numeroVertices, numeroMaximoLinhas, numeroMaximoColunas, linhaPonto, colunaPonto;
	...
	nArgumentos = 7 + (2 * strtoul (argv[3], &verificacao, 10));

	if (argc != nArgumentos)
	{
		printf ("Erro: Numero de argumentos invalido. \n");
		printf ("Uso: %s <Insira o numero de linhas e colunas do monitor, o numero de vertices, linha e coluna de cada vertice, linha e coluna do Ponto e o tempo de congelamento.> \n", argv [0]);
	...
	numeroVertices = strtoul (argv[3], &verificacao, 10);
	linhaPonto = strtoul (argv [nArgumentos - 3], &verificacao, 10);
	colunaPonto = strtoul (argv [nArgumentos - 2], &verificacao, 10);
	tempoEspera = strtoul (argv [nArgumentos - 1], &verificacao, 10);
	...
	if (linhaPonto == 0 || colunaPonto == 0)
	{
		printf ("Erro: Linha/Coluna do Ponto eh nula. \n");
		exit (ARGUMENTO_INVALIDO);
	}
	...
	indiceAuxiliar = 0;
	for (indice = 4 ; indice < nArgumentos - 3 ; indice += 2)
	{
	...
	indiceAuxiliar = 0;
	for (indice = 5 ; indice < nArgumentos - 3; indice += 2)
	{
  ...
  if (codigoRetorno != ok)
  {
	...
	/* FUNCAO PREENCHER POLIGONO */
  codigoRetorno = PreencherPoligono (monitor, numeroMaximoLinhas, numeroMaximoColunas, linhaPonto, colunaPonto, tempoEspera);
  if (codigoRetorno != ok)
  {
		if (codigoRetorno == pixelInvalido) {
			printf ("Erro: Pixel defeituoso, nao foi possivel preencher o plano/poligono. \n");
			exit (ERRO_FUNCAO_PREENCHER_POLIGONO);
		}
		else {
			printf ("Funcao PreencherPoligono retornou o erro: %i\n", codigoRetorno);
			exit (ERRO_FUNCAO_PREENCHER_POLIGONO);
		}
  }
  ...
  if (codigoRetorno != ok)
  {
	...
	/* 
		...
		TESTE 2 - PIXELS APAGADOS, ACESOS E COM DEFEITO
